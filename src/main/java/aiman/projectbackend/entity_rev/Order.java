...
public class Order {
    ...
    /**
     * Definisco i campi che andranno comporre il mio order:
     * - ID
     * - data ordine
     * - ammontare totale
     * - status
    ...
    // ManyToOne --> stabilisco la relazione, dicendo n ordini associato 1 utente
    @ManyToOne
    // JoinColum --> è la mia foreign key per stabilire la relazione
    @JoinColumn(name = "user_id")
    private User user;

    // OneToMany --> stabilisco la relazione, dicendo 1 ordine associato n prodotti (lista prodotti)
    // mappeBy --> mi serve per indicare la dominanza in questa relazione, ad esempio Order domina sulla lista prodotti ovvero OrderItem, se cancello lui allora cancello la sua lista ordine
    // questo mi evita di creare colonne in più anche, perchè JPA penserà che abbia due relazioni separate
    // quindi avrò una relazione bidirezionale e la foreign key è presente solo nel lato dominato OrderItem
    // CascadeType.All --> mi serve per gestire l'eliminazione se elimino un 1 ordino allora elimino le righe d'ordine o lista prodotti associati all'ordine su orderItem
    ...
    // Questo costruttore mi serve per creare i miei oggetti order
    public Order(LocalDateTime orderDate, Double totalAmount, String status, User user) {
